...
You MUST return ONLY a valid JSON object matching this exact structure:
{
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 2000,
      system: systemPrompt,
      messages: [{ role: "user", content: \The user swiped right on these foods: \. Combine their flavor profiles into a mind-blowing fusion dish.\ }]
    });
...
Language: \. Return ONLY a JSON object matching the Recipe structure (with title, description, ingredients with cheap prices in amount field, steps).\;
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 2000,
      system: systemPrompt,
      messages: [{ role: "user", content: \Create a full meal recipe for under ₹\. It must be delicious but use very cheap local Indian ingredients.\ }]
    });
    const textContent = response.content.find(c => c.type === 'text')?.text || "{}";
    const cleanJson = textContent.replace(/\\\json\n?/g, '').replace(/\\\\n?/g, '').trim();
    return JSON.parse(cleanJson) as Recipe;
...
export async function routeFridgeScanToGemini(imageBase64: string, language: AppLanguage): Promise<string> {
  try {
    ...
    const result = await model.generateContent([\List raw ingredients (max 10) in \. Comma separated.\, { inlineData: { data: imageBase64, mimeType: "image/jpeg" } }]);
