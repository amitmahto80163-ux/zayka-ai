...
export default function HomeClient({ initialDishes }: { initialDishes: any[] }) {
  ...
  // 1. Filter initialDishes based on category
  const combinedDishes = [...initialDishes, ...generatedDishes];
  ...
  const recommendedRecipes = initialDishes.filter((r: any) => {
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/profile">
              <div style={{
                ...
                {user?.photoUrl
                  ? <Image src={user.photoUrl} alt="Avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} width={400} height={400} />
      ...
      <div className="px-6 pb-24 flex flex-col gap-10 mt-2">
              ...
              {recommendedRecipes.slice(0, 5).map((recipe: any) => (
                <Link href={`/recipe/${recipe.id}`} key={`rec-${recipe.id}`}>
                  <motion.div whileTap={{ scale: 0.95 }} style={{
                    ...
                    <Image src={recipe.image} alt={recipe.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} width={400} height={400} />
                    <div style={{
        ...
        <div>
          <h2 style={{ fontSize: 18, fontWeight: 800, color: W.text, marginBottom: 16 }}>
            {activeCategory === 'all' ? `Trending Today 🚀 (${initialDishes.length}+ dishes)` : 'Found Recipes 🍳'}
          </h2>
            ...
            <AnimatePresence>
              {displayedDishes.map((recipe: any) => (
                <Link href={`/recipe/${recipe.id}`} key={recipe.id}>
                  ...
                  >
                    <Image src={recipe.image} alt={recipe.name} loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }} width={400} height={400} />
                    <div style={{
